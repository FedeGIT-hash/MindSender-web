...
const RotatingText = forwardRef<any, RotatingTextProps>((props, ref) => {
  ...
  const elements = useMemo(() => {
    ...
    return splitBy === 'words'
      ...
      : currentText.split(splitBy).map((part, i, arr) => ({
          characters: [part],
          needsSpace: i !== arr.length - 1,
        }))
  ...
  useImperativeHandle(
    ...
    [next, previous, jumpTo, handleIndexChange]
  )
    ...
    >
      ...
      <AnimatePresence mode={animatePresenceMode} initial={false}>
        <motion.div
          ...
          className={`flex flex-wrap whitespace-pre-wrap ${splitLevelClassName}`}
          layout
        >
          {elements.map((wordObj, wordIndex, array) => {
            ...
            return (
              <span key={wordIndex} className="inline-flex flex-wrap whitespace-pre-wrap">
